...
def main(spark, userID, file_size):
    ...
    user_subset = indexed_val.select("user_id").distinct()
    ...
    indexed_val = indexed_val.select(col("user_id"), col("recording_index").alias("label"))
    indexed_val = indexed_val.groupBy("user_id").agg(expr("collect_list(label) as labels"))
    ...
    user_rec = user_rec.join(indexed_val, "user_id", "inner")
    ...
    # Print the total time taken for the job
    print(f"\nTotal time taken for the job: {final_time} seconds.\n")
    ...
    with open('output.txt', 'w') as f:
        ...
        f.write(f"Total time taken for the job: {final_time} seconds.\n")
